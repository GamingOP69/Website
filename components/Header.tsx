...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 mb-5 -mx-4 border-b border-gray-800 bg-bg/92 px-4 py-3 backdrop-blur-md sm:-mx-6 sm:mb-7 sm:px-6 lg:-mx-8 lg:px-8">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <button
          ...
          className="md:hidden flex flex-col gap-1.5 w-9 h-9 justify-center items-center rounded-md border border-gray-800 bg-white/5"
      ...
      {/* Mobile Navigation */}
      {isOpen && (
        <nav className="md:hidden mt-3 grid grid-cols-2 gap-2 border-t border-gray-800 pt-3 pb-1">
          {NAV_ITEMS.map((item) => {
            const isActive = isActiveHref(item.href)
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`block rounded-md px-3 py-2.5 text-sm transition-all no-underline ${
                  isActive
